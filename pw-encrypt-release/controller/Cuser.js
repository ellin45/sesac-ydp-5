...
const db = require('./models/index'); // ./models/index.js
...
exports.getProfile = async (req, res) => {
  ...
  if(id === userInfo.id && pw === userInfo.pw){
    req.session.name = `${id}`;
    res.render('profile',{user:req.session.name, isLogin: true});
  }
};
...
exports.postRegister = async (req, res) => {
  ...
  db.sequelize.sync({ force: false }).then(() => {
    ...
    // force: true; 데이터베이스에 테이블이 있어도 무조건 생성
    app.listen(PORT, () => {
